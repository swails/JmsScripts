...
from chemistry.amber.constants import RAD_TO_DEG
...
class _DihedralTerm(object):
   ...
   def __init__(self, idivf=1, pk=None, phase=None, periodicity=None,
      ...
      if dihedraltype is not None:
         if not isinstance(dihedraltype, DihedralType):
            raise TypeError('dihedraltype expected to be DihedralType instance')
   ...
   def __eq__(self, other):
      if self.dihtype != other.dihtype and 'improper' in (self.dihtype,
                                          other.dihtype):
         return False
      return (abs(self.type.phi_k - other.type.phi_k) < _TINY and
              abs(self.type.phase - other.type.phase) < _TINY and
              abs(self.type.per - other.type.per) < _TINY)
...
class DihedralParamList(ParamList):
   ...
   def add_param(self, atype1, atype2, atype3, atype4, param):
      ...
      terms, so adding a DihedralParam to a DihedralParamList is different than
      the other parameter lists
      """
      if not isinstance(param, self.type):
         raise TypeError('Added parameter should be type %s' %
                         self.type.__name__)
      # Loop through all DihedralParam objects in this list. Add it to any
      # DihedralParam that has the same atoms. If it's a duplicate term, it will
      # be discarded.
      added = False
      for oparam in self:
         if oparam.same_atoms((atype1, atype2, atype3, atype4))[0]:
            # if one is an improper and the other is not, skip over this. If
            # both are impropers, let it through to bre screened for equality
            # If both are propers, also let them through.
            if ((oparam[0].dihtype == 'improper' and param.dihtype == 'normal')
                     or (oparam[0].dihtype == 'normal' and 
      ...
      for i, dihedral in enumerate(parm.dihedrals_without_h):
         ...
         try:
            self._add_dihedral(dihedral.atom1.attype, dihedral.atom2.attype,
                     dihedral.atom3.attype, dihedral.atom4.attype, idivf=1,
                     dihedraltype=dihedral.dihed_type, dihtype=term)
         except Exception, err:
            print 'Problem adding dihedral %d' % i
            print '\t%s-%s-%s-%s' % (dihedral.atom1.attype, dihedral.atom2.attype,
